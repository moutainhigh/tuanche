        ...
        <div class="ibox-content">
            <div class="row">
                <div class="form-group">
                    <input type="hidden" id="packageId" value="${id}">
    ...
    <div class="float-e-margins">
        <div class="ibox-content">
            <div class="row row-lg">
                <!-- Example Pagination -->
                <div class="col-sm-12">
                    <button class="btn btn-primary" type="button" data-toggle="modal" style="height:30px;background: #1ab394;border-color: #1ab394" data-target="#myModal5">
                        新增菜品
                    </button>
                    <table id="listTable" class="table table-bordered" data-click-to-select="true"
                        ...
                        <tr>
                            ...
                            <th data-field="handle" data-width="15%" data-align="center"
                                data-formatter="formatOperate2"><span class="tdfont">操作</span></th>
                        </tr>
                        </thead>
                    </table>
                </div>
...
<div class="modal inmodal" id="myModal5" tabindex="-1" role="dialog" aria-hidden="true">
    <div style="z-index: 9998" class="modal-dialog modal-lg">
        <div class="modal-content">
                ...
                <div class="ibox-content">
                    <div class="row">
                        <div class="form-group">
                            ...
                            <label class="col-sm-1 control-label mtop">菜&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;名:</label>
                            <div class="col-sm-2">
                                <input id="productNameS2" type="text" value="" class="form-control">
                            </div>
                            <div class="col-sm-1">
                                <button class="btn btn-primary" type="button" onclick="query2();">
                                    <i class="fa fa-search"></i>&nbsp;搜索
                                </button>
                            </div>
                        </div>
                    ...
                    <div class="row row-lg">
                        ...
                        <div class="col-sm-12">
                            <table id="listTable2" class="table table-bordered" data-click-to-select="true"
                                ...
                                <thead>
                                <tr>
                                    <th data-field="hospitalSn" data-checkbox="true" data-width="15%">请选择</th>
                                    <th data-field="id" data-width="10%"
                                        data-align="center"><span class="tdfont">ID</span></th>
                                    <th data-field="productName" data-width="10%"
                                        data-align="center"><span class="tdfont">菜单名称</span></th>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-primary" onclick="getSelect()" data-dismiss="modal">保存
                </button>
                <button type="button" class="btn btn-white" data-dismiss="modal">关闭</button>
            </div>
        </div>
...
<script>
    ...
    function paginationParam2(params) {
        var openTime = $("#openTime2").val();
        var tillTime = $("#tillTime2").val();
    ...
    function formatOperate2(value, row, index) {
        var result = "";
        result = result + "<a title='编辑' onclick='deleteProduct(" + row.id + ")'  data-toggle='modal' data-target='#editModal')>删除</a>&nbsp;&nbsp;&nbsp;&nbsp;";
        return result;
    }
    //编辑菜品
    function toedit(id) {
        $.ajax({
            ...
            success: function (result) {
                ...
                if (result.code === 0) {
                    ...
                    $('#productClassify_e').val(result.data.productClassify);
                    $('#windowCode_e').val(result.data.windowCode);
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.code === 0) {
                    ...
                    $('#productClassify_d').val(result.data.productClassify);
                    $('#windowCode_d').val(result.data.windowCode);
...
function deleteProduct(id) {
        $.ajax({
            ...
            type: "post",
            dataType: "json",
            url: 'productPackage/deleteProduct',
            success: function (result) {
                if (result.code === 0) {
                    layer.alert("操作成功", {icon: 6, time: 2000, title: '提示'});
                    $('#listTable').bootstrapTable('refresh', {silent: true, query: {pageNumber: 1}});
                } else {
                    layer.alert(result.msg, {icon: 5, time: 2000, title: '提示'});
                    $("#saveBtn").removeAttr("disabled");
                }
    ...
    function saveProduct() {
        ...
        ;
        if ($("#productClassify_a option:selected").val() == null || $("#productClassify_a option:selected").val() == "") {
        ...
        $.ajax({
            data: {
                ...
                'productClassify': $("#productClassify_a option:selected").val(),
                'windowCode': $("#windowCode_a option:selected").val(),
    ...
    function editSaveProduct() {
        ...
        ;
        if ($("#productClassify_e option:selected").val() == null || $("#productClassify_e option:selected").val() == "") {
            layer.alert("请选择分类", {icon: 5, time: 2000, title: '提示'});
            $("#saveBtnE").removeAttr("disabled");
            return false;
        }
        ...
        $.ajax({
            data: {
                ...
                'productClassify': $("#productClassify_e option:selected").val(),
                'windowCode': $("#windowCode_e option:selected").val(),
                'productType': productType,
    ...
    function getSelect() {
        var selectVar = $('#listTable2').bootstrapTable('getSelections');
        $.ajax({
            data: {
                'productId': selectVar[0].id,
                'packageId': $("#packageId").val(),
            },
            type: "post",
            dataType: "json",
            url: 'productPackage/addPackageProduct',
            success: function (result) {
                if (result.code === 0) {
                    layer.alert("操作成功", {icon: 6, time: 2000, title: '提示'});
                    $('#listTable').bootstrapTable('refresh', {silent: true, query: {pageNumber: 1}});

		...
		<div class="ibox-content">
			<div class="row" style="height:49px;">
				<div class="form-group">
					<label class="col-sm-1 control-label mtop">分类名称:</label>
					<div class="col-sm-2">
						<input id="classifyName_s" type="text" value=""
							   class="form-control">
					</div>
					<div class="col-sm-1">
	...
	<div class="ibox float-e-margins">
		<div class="ibox-content">
			<div class="row row-lg">
				<div class="col-sm-12">
					<button id="addMenuButton" type="button"  class="btn btn-primary" data-toggle="modal" data-target="#addModal">添加分类</button>
					<div class="example-wrap">
						<div class="example">
							<table id="listTable" class="table table-bordered" data-click-to-select="true"
								   ...
								   data-height="470"
								   data-url="classify/showClassifyList">
								<thead>

								<tr>
									<th data-field="id" data-visible="false"></th>
									<th data-field="classifyName" data-width="15%"
										data-align="center"><p class="tdfont">分类名称</p></th>
									<th data-field="rank" data-width="15%"
										data-align="center"><p class="tdfont">排序号</p></th>
					...
					<div class="ibox-content">
						<form id="form" class="form-horizontal m-t" >
							...
							<div class="form-group">
								...
								<div class="col-sm-8">
									<input id="classifyName_a" name="realName" type="text" class="form-control">
								</div>
							</div>
								<div class="form-group">
								<label class="col-sm-3 control-label">排序号:</label>
								<div class="col-sm-8">
									<input id="rank_a" name="realName" type="text" class="form-control">
								</div>
							</div>
					...
					<div class="ibox-content">
						<form id="form" class="form-horizontal m-t" >
							...
							<div class="form-group">
								<label class="col-sm-3 control-label">分类名称:</label>
								<div class="col-sm-8">
									<input id="classifyName_e" name="realName" type="text" class="form-control">
								</div>
							</div>
							<div class="form-group">
								<label class="col-sm-3 control-label">排序号:</label>
								<div class="col-sm-8">
									<input id="rank_e" name="realName" type="text" class="form-control">
								</div>
							</div>
							<!-- 用于 将表单缓存清空 -->
...
<script>
    ...
    function paginationParam(params) {
        return {
            ...
            classifyName : $.trim($('#classifyName_s').val()),
        };
    }
    function query() {
    ...
    function goUpdateMedicalAgent(id,classifyName,rank){
        $('#updateReset').trigger("click");
        $('#idE').val(id);
        $('#rank_e').val(rank);
        $('#classifyName_e').val(classifyName);
        if(isDel == 0) {
    ...
    function editMedicalAgent() {
        ...
        if ($("#classifyName_e").val() == ""){
            layer.alert("请填写分类名称", {icon: 5,time: 2000, title:'提示'});
            $("#editBtn").removeAttr("disabled");
            return false;
        }if ($("#rank_e").val() == ""){
            layer.alert("请填写排序号", {icon: 5,time: 2000, title:'提示'});
            $("#editBtn").removeAttr("disabled");
            return false;
        }
        ...
        $.ajax({
            data : {
                "id" : $("#idE").val(),
                'classifyName' : $("#classifyName_e").val(),
                'rank' : $("#rank_e").val(),
            },
            type : "post",
            dataType : "json",
    ...
    function formateOperate(value, row, index) {
        var result = "<a  onclick='goUpdateMedicalAgent(\""+ row.id + "\",\""+ row.classifyName + "\",\""+ row.rank + "\");' data-toggle='modal' data-target='#updateModal' >编辑</a>&nbsp;&nbsp;&nbsp;&nbsp;";
    ...
    function saveApp() {
        ...
        $.ajax({
            data : {
                'classifyName' : $("#classifyName_a").val(),
                'rank' : $("#rank_a").val(),
            },
            type : "post",
            dataType : "json",

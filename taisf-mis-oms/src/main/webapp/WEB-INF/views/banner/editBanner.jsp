		...
		<div class="col-sm-12">
			<div class="ibox float-e-margins">
				...
				<div class="ibox-content">
					<input type="hidden" class="form-control disabled" id="id" name="id" value="${entity.id}">
<!-- 					<input id="appCode" type="hidden" value="jkHospital" class="form-control"> -->
					<form id="form" class="form-horizontal m-t" >
						<div class="form-group">
							...
							<div class="col-sm-8">
								<input type="text" class="form-control disabled" id="title" name="title" value="${entity.title}">
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-sm-8">
								<select id="appCode" name="appCode" class="form-control" onchange="getAppPositionByCode()">
										...
										<%--<option value="jkHospital" <c:if test="${entity.appCode == 'jkHospital'}">selected</c:if>>健客医院</option>--%>
										<%--<option value="jkCloudClinic" <c:if test="${entity.appCode == 'jkCloudClinic'}">selected</c:if>>健客诊所</option>--%>
										<c:forEach var="app" items="${bannerAppList}">
											<option value="${app.appCode}" <c:if test="${entity.appCode == app.appCode}">selected</c:if>>${app.text}</option>
										</c:forEach>
								</select>
							</div>
						</div>
						...
						<div class="form-group">
							...
							<c:if test="${not empty entity.position}">
								<input id="position" type="hidden" value="${entity.position}"/>
							</c:if>
							<div class="col-sm-8">
						...
						<div class="form-group">
							...
							<div class="col-sm-8">
								<div class="radio i-checks">
									<label>
										<input type="radio" value="1" name="platform"  <c:if test="${entity.platform == 1}">checked</c:if> > <i></i>iOS</label>
									<label>
										<input type="radio" value="2" name="platform"  <c:if test="${entity.platform == 2}">checked</c:if> > <i></i>Android</label>
									<label>
										<input type="radio" value="3" name="platform"  <c:if test="${entity.platform == 3}">checked</c:if> > <i></i>m站</label>
									<label>
										<input type="radio" value="4" name="platform"  <c:if test="${entity.platform == 4}">checked</c:if> > <i></i>微信</label>
									<label>
										<input type="radio" value="5" name="platform"  <c:if test="${entity.platform == 5}">checked</c:if> > <i></i>微信小程序</label>
								</div>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-sm-8">
								<input type="text" class="form-control disabled" id="rank" name="rank" value="${entity.rank}">
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-sm-8">
								<div class="radio i-checks">
									<label>
										<input type="radio" value="1" name="jumpType" <c:if test="${entity.jumpType == 1}">checked</c:if>> <i></i>原生</label>
									<label>
										<input type="radio" value="2" name="jumpType" <c:if test="${entity.jumpType == 2}">checked</c:if>> <i></i>h5</label>
								</div>
							</div>
						</div>
						<div class="form-group">
							<label class="col-sm-3 control-label"><font>*&nbsp;</font>开始时间:</label>
							<input type="hidden" id="startTimeDate" value="${entity.startTime}">
							<div class="col-xs-2 col-sm-2">
								<input id="startTime"  name="startTime" value="" class="laydate-icon form-control layer-date">
							</div>

							<input type="hidden" id="endTimeDate" value="${entity.endTime}">
						...
						<div class="form-group">
							...
							<div class="col-sm-8">
								<input type="text" class="form-control disabled" id="url" name="url" value="${entity.url}">
							</div>
						</div>
						<div class="form-group">
							<label class="col-sm-3 control-label"><font>*&nbsp;</font>图片:</label>
							<div class="col-sm-8">
								<a id="image11-1" href="${entity.imageUrl }" title="图片" data-gallery="">
									<img id='imgSizeImgSrc1' style="margin-left:50px" width="400" height="400" src="${entity.imageUrl }" /></a>
								<input id='imgUrl1' type="hidden" name="imageUrl" value="${entity.imageUrl}"/>
								<a  href="javascript:;" style="top:70" class="file">
									<input  type="file" onchange="uploadPic(this)" name="pics" multiple="multiple"/>
									上传图片
								</a>
							</div>
						</div>
					...
					<div class="row">
						...
						<div class="col-sm-2">
							<button class="btn btn-primary" id="saveBtn" type="button" onclick="saveBanner();">保存</button>
						</div>
						<div class="col-sm-1">
							<button class="btn btn-primary" id="saveBannerAndUpBtn"  type="button" onclick="saveBannerAndUp();">保存并上架</button>
						</div>
					</div>
				</div>
			</div>
...
<script type="text/javascript">
    ...
    function saveBanner() {
        ...
        CommonUtils.ajaxPostSubmit("banner/updateBanner", {
                "id" : $("#id").val(),
                "appCode":$("#appCode").val(),
                "title" : $("#title").val(),
        ...
        $.ajax({
            ...
            success:function (result) {
                ...
                $(positionList).each(function(i,e){
                    ...
                    if(position==e.positionCode){

                        $("#appPosition").append("<option value='"+e.positionCode+"'selected='selected'>"+e.postionName+"</option>")
    ...
    $(function(){
        ...
        $(function (){
            ...
            var startTime = $("#startTimeDate").val();

            if( startTime != null && startTime !=''){

                var startTimeDate=startTime.split(" ")[0];

                $("#startTime").val(startTimeDate);

            }

            var endTime = $("#endTimeDate").val();

            if( endTime != null && endTime !=''){

                var endTimeDate=endTime.split(" ")[0];
